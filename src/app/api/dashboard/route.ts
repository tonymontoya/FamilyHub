...
export const GET = withFlatErrorHandling(async (request) => {
  ...
  try {
    ...
    throw Errors.badRequest("Invalid timezone")
  }
  ...
  // NOTE(Finding 5): the child filter below intentionally hides unassigned
  // chores once another child has claimed them (first-come-first-served). The
  // "correct" alternative (show every unassigned chore, derive per-child status)
  // is a product-semantics question left open, not a clear bug. Preserved as-is.
